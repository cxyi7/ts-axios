...
describe('helpers:headers', () => {
  describe('parseHeaders', () => {
    ...
    test('should return empty object if headers is a empty string', () => {
  ...
  describe('processHeaders', () => {
    test('should normalize Content-Type header name', () => {
      ...
      expect(headers['Content-Type']).toBe('foo/baz')
      expect(headers['conTenT-Type']).toBeUndefined()
      expect(headers['Content-length']).toBe(1024)
    })

    test('should set Content-Type if not set and data is Plainoject', () => {
      const headers: any = {}

...
describe('instance', () => {
  ...
  test('should make  a http request without helper', () => {
  ...
  test('should have interceptors on the instance', done => {
    ...
    instance.get('/foo').then(res => (respose = res))
    ...
    setTimeout(() => {
      expect(respose.config.timeout).toBe(0)
      expect(respose.config.withCredentials).toBe(true)
      done()
    }, 100)
  })
